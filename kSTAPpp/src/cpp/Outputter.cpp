		...
		*this << " NUMBER OF ELEMENTS. . . . . . . . . . .( NPAR(2) ) . . =" << setw(5) << NUME
		...
		switch (ElementType)
		{
			case ElementTypes::Bar: // Bar element
				...
				break;
			case ElementTypes::CD32: //  element
				OutputBarElements(EleGrp);
				break;
		    default:
...
void COutputter::PrintMassMatrix()
{
	...
	unsigned int NEQ = FEMData->GetNEQ();
	CMasslineMatrix<double> *MassMatrix = FEMData->GetMassMatrix();
	unsigned int* DiagonalAddress = MassMatrix->GetMassDiagonalAddress();
//	for (unsigned int i = 0; i < DiagonalAddress[NEQ] - DiagonalAddress[0]; i++)
//	{
//		*this << setw(14) << (*MassMatrix)(i,0);
	...
	*this << "*** _Debug_ *** Full mass matrix" << endl;
	...
	for (int I = 1; I <= NEQ; I++)
	{
		for (int J = 1; J <= NEQ; J++)
		{
			...
			{
				*this << setw(14) << (*MassMatrix)(I_new, J_new);
			}
		}
