...
//! constructors
    inline CMasslineMatrix();
    inline CMasslineMatrix(unsigned int N);
...
//! For the sake of efficiency, the index bounds are not checked
    inline P_& operator()(unsigned int i, unsigned int j);

#ifdef _DEBUG_
//! operator (i) where i numbering from 1
//! For the sake of efficiency, the index bounds are not checked
    inline P_& operator()(unsigned int i);
#endif
    
//! Allocate storage for the skyline matrix
    inline void MassAllocate();
    
//! Calculate the column height, used with the skyline storage scheme
    void CalculateMassColumnHeight(unsigned int* LocationMatrix, size_t ND);

//! Calculate the maximum half bandwidth ( = max(ColumnHeights) + 1 )
    void CalculateMassMaximumHalfBandwidth();

//! Calculate address of diagonal elements in banded matrix
//! Caution: Address is numbered from 1 !
    void CalculateMassDiagnoalAddress();

//! Assemble the element stiffness matrix to the global stiffness matrix
    void AssemblMass(double* Matrix, unsigned int* LocationMatrix, size_t ND);

//! Return pointer to the ColumnHeights_
    inline unsigned int* GetMassColumnHeights();

//! Return the maximum half bandwidth
    inline unsigned int GetMassMaximumHalfBandwidth() const;

//! Return pointer to the DiagonalAddress_
    inline unsigned int* GetMassDiagonalAddress();

//! Return the dimension of the stiffness matrix
    inline unsigned int Massdim() const;
    
//! Return the size of the storage used to store the stiffness matrkix in skyline
    inline unsigned int Masssize() const;

}; /* class definition */

//! constructor functions
template <class P_>
inline CMasslineMatrix<P_>::CMasslineMatrix()
{
    NEQ_ = 0;
    MK_  = 0;
    NWK_ = 0;
...
template <class P_>
inline CMasslineMatrix<P_>::CMasslineMatrix(unsigned int N)
{
    NEQ_ = N;
    MK_  = 0;
    NWK_ = 0;
...
//! destructor function
template <class P_>
inline CMasslineMatrix<P_>::~CMasslineMatrix<P_>()
{
    if (ColumnHeights_)
...
template <class P_>
inline P_& CMasslineMatrix<P_>::operator()(unsigned int i, unsigned int j)
{
    if (j >= i)
...
template <class P_>
inline P_& CMasslineMatrix<P_>::operator()(unsigned int i)
{
    return data_[i];
}
#endif
...
template <class P_>
inline void CMasslineMatrix<P_>::MassAllocate()
{
...
//! Return pointer to the ColumnHeights_
template <class P_>
inline unsigned int* CMasslineMatrix<P_>::GetMassColumnHeights()
{
...
//! Return the maximum half bandwidth
template <class P_>
inline unsigned int CMasslineMatrix<P_>::GetMassMaximumHalfBandwidth() const
{
    return(MK_);
}
...
//! Return pointer to the DiagonalAddress_
template <class P_>
inline unsigned int* CMasslineMatrix<P_>::GetMassDiagonalAddress()
{
...
//! Return the dimension of the stiffness matrix
template <class P_>
inline unsigned int CMasslineMatrix<P_>::Massdim() const
{
    return(NEQ_);
}
...
//! Return the size of the storage used to store the stiffness matrkix in skyline
template <class P_>
inline unsigned int CMasslineMatrix<P_>::Masssize() const
{
   return(NWK_);
}
...
//  Calculate the column height, used with the skyline storage scheme
template <class P_>
void CMasslineMatrix<P_>::CalculateMassColumnHeight(unsigned int* LocationMatrix, size_t ND)
{    
...
template <class P_>
void CMasslineMatrix<P_>::CalculateMassMaximumHalfBandwidth()
{
...
template <class P_>
void CMasslineMatrix<P_>::AssemblMass(double* Matrix, unsigned int* LocationMatrix, size_t ND)
{   
...
//      Address of diagonal element of column j in the one dimensional element stiffness matrix
        ...
        for (unsigned int i = 0; i <= j; i++)
        {
            ...
            if (!Li) continue;
            //std::cout << DiagonalAddress_ <<std::endl;
...
//    Caution: Address is numbered from 1 !
template <class P_>
void CMasslineMatrix<P_>::CalculateMassDiagnoalAddress()
{
